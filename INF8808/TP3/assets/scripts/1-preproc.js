...
function initializeData(data) {
  // TODO: Convertir les propriétés "income", "lifeExpectancy" et "population" au format "number" pour chacune des entrées.
    data.forEach(param => {
      ...
      param.income = parseFloat(param.income);

      param.lifeExpectancy = parseFloat(param.lifeExpectancy);

      param.population = parseInt(param.population);
...
function domainColor(color, data) {
  ...
  //color.domain(data.columns.slice(2));
 
  
  var zoneDis;

  zoneDis= Array.from(new Set(data.map(function(elem){
...
function domainRadius(r, data) {
  ...
  // TODO: Préciser le domaine de l'échelle de la variable "r" em spécifiant comme valeurs extrêmes le minimum et le maximum des populations des pays.

  //valeurs maximum des populations des pays.
  var maximumPopulation;
  maximumPopulation= d3.max(data, function(d){
    return d.population
  });

  //valeurs minimum des populations des pays.
  var minimumPopulation; 
  minimumPopulation = d3.min(data, function(d){
    return d.population
  });
  r.domain([minimumPopulation, maximumPopulation])
}
