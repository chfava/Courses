...
def calculateScorePHQ9(questions_PHQ9):
        return np.sum(np.array(questions_PHQ9))

def calculateScoreGAD(questions_GAD):
        return np.sum(np.array(questions_GAD))

def calculateScoreOBC(questions_OBC):
        return np.sum(np.array(questions_OBC))

def calculGCPS(questions_GCPS):
        del questions_GCPS[4]
        return np.sum(np.array(questions_GCPS[1:]))

def predictTreatment(diagnosticVector, MD1, QS5,  questions_OBC, questions_GAD, questions_PHQ9, questions_GCPS):
        RETURN = {}
        SCORE_VALUE_PHQ9 = 9
        SCORE_VALUE_OBC = 24
        SCORE_VALUE_GAD = 9
        ...
        vectorLength = len(diagnosticVector)
        numberOfDiagnostic = int(vectorLength / 12)

        OBC1 = questions_OBC[0]
        OBC4 = questions_OBC[3]
        OBC5 = questions_OBC[4]
        OBC6 = questions_OBC[5]

        serreDents = (OBC1 != 0 or OBC4 != 0 or OBC5 != 0 or OBC6 != 0)
        diagnostic = np.array(diagnosticVector)
        if all(diagnostic == 0) :
                RETURN['E13_pred'] = ['F', 1]
                return RETURN

        if(calculateScorePHQ9(questions_PHQ9) > SCORE_VALUE_PHQ9 and calculateScoreGAD(questions_GAD) > SCORE_VALUE_GAD and calculateScoreOBC(questions_OBC) > SCORE_VALUE_OBC):
                RETURN['E13_TCC_pred'] = ['T', 1]
                RETURN['E13_PSY_pred'] = ['T', 1]
        if (calculGCPS(questions_GCPS) > SEUIL_GCPS): 
                RETURN['E13_HYP_PRED']= ['T', 1]
                RETURN['E13_TP_PRED']= ['T', 1]
        if MD1 < 30:
                RETURN['E13_ET_PRED']= ['T', 1]
        elif MD1 < 180:
                for index in range(1, numberOfDiagnostic + 1):
                        #Myalgie
                        if diagnosticVector[1 * index] == 1:
                                if (diagnosticVector[0] == 1 or serreDents or QS5):
                                        RETURN['E13_OO_PRED'] = ['T', 1]
                                        RETURN['E13_KIN_PRED'] = ['T', 1]
                                else:
                                        RETURN['E13_KIN_PRED'] = ['T', 1]
                        #Arthralgie
                        if diagnosticVector[5 * index] == 1:
                                RETURN['E13_OO_PRED'] = ['T', 0.7]
                                RETURN['E13_KIN_PRED'] = ['T', 0.7]
                ...
                for index in range(1, numberOfDiagnostic + 1):
                        ...
                        if diagnosticVector[1 * index] == 1:
                                if (diagnosticVector[0] == 1 or serreDents or QS5):
                                        RETURN['E13_OO_PRED'] = ['T', 1]
                                        RETURN['E13_KIN_PRED'] = ['T', 1]
                                        RETURN['E13_HYP_PRED']= ['T', 0.6]
                                else:
                                        RETURN['E13_KIN_PRED'] = ['T', 1]
                                        RETURN['E13_HYP_PRED']= ['T', 0.6]
                        #Arthralgie
                        if diagnosticVector[5 * index] == 1:
                                RETURN['E13_OO_PRED'] = ['T', 0.9]
                                RETURN['E13_KIN_PRED'] = ['T', 0.9]
                                RETURN['E13_HYP_PRED']= ['T', 0.6]
        if MD1 > 0:
                for index in range(1, numberOfDiagnostic + 1):
                        #Subluxation
                        if diagnosticVector[11 * index] == 1:
                                RETURN['E13_ET_PRED']= ['T', 1]
                                RETURN['E13_KIN_PRED'] = ['T', 1]
...
def preprocess(dataQuestionnaires):
    ...
    def preprocess_OBC_1_2(x):
            ...
            if x == '1-3 Nuit/mois':
                    return 2
            if x == '1-3 Nuit/semaine':
                    return 3
            if x == '4-7 Nuit/semaine':
                    return 4
            else:
                    return 0

    def preprocess_OBC_3_21(x):
            if x == 'Jamais':
                    return 0
            if x == '1 Nuit/mois' or x == 'Un peu de temps':
                    return 1
            if x == '1-3 Nuit/mois' or x == 'Une partie du temps':
                    return 2
            if x == '1-3 Nuit/semaine' or x == 'La pluspart du temps' or x == 'La plupart du temps':
                    return 3
            if x == '4-7 Nuit/semaine' or x == 'Tout le temps':
                    return 4
            else:
                    return 0

    def preprocess_GAD(x):
            if pd.isnull(x):
                    return 0
            if str(x) == 'F':
                    return 0
    ...
    ####################################################################################################################
    ####################################################################################################################
    ##Transformations pour TRAITEMENT
    ####################################################################################################################
    ####################################################################################################################

    df_TRAITEMENT = pd.DataFrame()
    df_TRAITEMENT['QS5'] = df['QS5'].apply(cleanOUI_NON_TO_TRUE_FALSE)
    df_TRAITEMENT['MD1'] = df['MD1'].apply(preprocess_MD1)

    df_TRAITEMENT['OBC1'] = df['OBC1'].apply(preprocess_OBC_1_2)
    df_TRAITEMENT['OBC2'] = df['OBC2'].apply(preprocess_OBC_1_2)
    df_TRAITEMENT['OBC3'] = df['OBC3'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC4'] = df['OBC4'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC5'] = df['OBC5'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC6'] = df['OBC6'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC7'] = df['OBC7'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC8'] = df['OBC8'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC9'] = df['OBC9'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC10'] = df['OBC10'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC11'] = df['OBC11'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC12'] = df['OBC12'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC13'] = df['OBC13'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC14'] = df['OBC14'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC15'] = df['OBC15'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC16'] = df['OBC16'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC17'] = df['OBC17'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC18'] = df['OBC18'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC19'] = df['OBC19'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC20'] = df['OBC20'].apply(preprocess_OBC_3_21)
    df_TRAITEMENT['OBC21'] = df['OBC21'].apply(preprocess_OBC_3_21)

    df_TRAITEMENT['GAD1'] = df['GAD1'].apply(preprocess_GAD)
    df_TRAITEMENT['GAD2'] = df['GAD2'].apply(preprocess_GAD)
    df_TRAITEMENT['GAD3'] = df['GAD3'].apply(preprocess_GAD)
    df_TRAITEMENT['GAD4'] = df['GAD4'].apply(preprocess_GAD)
    df_TRAITEMENT['GAD5'] = df['GAD5'].apply(preprocess_GAD)
    df_TRAITEMENT['GAD6'] = df['GAD6'].apply(preprocess_GAD)
    df_TRAITEMENT['GAD7'] = df['GAD7'].apply(preprocess_GAD)

    df_TRAITEMENT['PHQ91'] = df['PHQ91'].apply(preprocess_PHQ9)
    df_TRAITEMENT['PHQ92'] = df['PHQ92'].apply(preprocess_PHQ9)
    df_TRAITEMENT['PHQ93'] = df['PHQ93'].apply(preprocess_PHQ9)
    df_TRAITEMENT['PHQ94'] = df['PHQ94'].apply(preprocess_PHQ9)
    df_TRAITEMENT['PHQ95'] = df['PHQ95'].apply(preprocess_PHQ9)
    df_TRAITEMENT['PHQ96'] = df['PHQ96'].apply(preprocess_PHQ9)
    df_TRAITEMENT['PHQ97'] = df['PHQ97'].apply(preprocess_PHQ9)

    df_TRAITEMENT['DC1'] = df['DC1'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC2'] = df['DC2'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC3'] = df['DC3'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC4'] = df['DC4'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC5'] = df['DC5'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC6'] = df['DC6'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC7'] = df['DC7'].apply(preprocess_GCPS)
    df_TRAITEMENT['DC8'] = df['DC8'].apply(preprocess_GCPS)
    ...
    Y_predicted = []
    for i in range(0, len(Y_ATM_GAUCHE_predicted)):
            line_predicted = []
            line_predicted += Y_TEMPORAL_DROIT_predicted[i]
            line_predicted += Y_TEMPORAL_GAUCHE_predicted[i]
            line_predicted += Y_MASSETER_DROIT_predicted[i]
            line_predicted += Y_MASSETER_GAUCHE_predicted[i]
            line_predicted += Y_ATM_DROIT_predicted[i]
            line_predicted += Y_ATM_GAUCHE_predicted[i]
            line_predicted += Y_DIGASTRIQUE_DROIT_predicted[i]
            line_predicted += Y_DIGASTRIQUE_GAUCHE_predicted[i]
            line_predicted += Y_PTERIGOIDIEN_DROIT_predicted[i]
            line_predicted += Y_PTERIGOIDIEN_GAUCHE_predicted[i]
            line_predicted += Y_SOUS_MANDIBULAIRE_DROIT_predicted[i]
            line_predicted += Y_SOUS_MANDIBULAIRE_GAUCHE_predicted[i]
            line_predicted += Y_TENDON_TEMPORAL_DROIT_predicted[i]
            line_predicted += Y_TENDON_TEMPORAL_GAUCHE_predicted[i]
            Y_predicted.append(line_predicted)

    TREATMENT_predicted = []
    for i in range(len(Y_ATM_GAUCHE_predicted)):
            questions_OBC = []
            questions_OBC.append(df_TRAITEMENT['OBC1'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC2'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC3'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC4'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC5'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC6'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC7'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC8'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC9'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC10'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC11'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC12'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC13'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC14'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC15'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC16'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC17'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC18'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC19'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC20'].iloc[i])
            questions_OBC.append(df_TRAITEMENT['OBC21'].iloc[i])

            questions_GAD = []
            questions_GAD.append(df_TRAITEMENT['GAD1'].iloc[i])
            questions_GAD.append(df_TRAITEMENT['GAD2'].iloc[i])
            questions_GAD.append(df_TRAITEMENT['GAD3'].iloc[i])
            questions_GAD.append(df_TRAITEMENT['GAD4'].iloc[i])
            questions_GAD.append(df_TRAITEMENT['GAD5'].iloc[i])
            questions_GAD.append(df_TRAITEMENT['GAD6'].iloc[i])
            questions_GAD.append(df_TRAITEMENT['GAD7'].iloc[i])

            questions_PHQ9 = []
            questions_PHQ9.append(df_TRAITEMENT['PHQ91'].iloc[i])
            questions_PHQ9.append(df_TRAITEMENT['PHQ92'].iloc[i])
            questions_PHQ9.append(df_TRAITEMENT['PHQ93'].iloc[i])
            questions_PHQ9.append(df_TRAITEMENT['PHQ94'].iloc[i])
            questions_PHQ9.append(df_TRAITEMENT['PHQ95'].iloc[i])
            questions_PHQ9.append(df_TRAITEMENT['PHQ96'].iloc[i])
            questions_PHQ9.append(df_TRAITEMENT['PHQ97'].iloc[i])
            
            questions_GCPS = []
            questions_GCPS.append(df_TRAITEMENT['DC1'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC2'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC3'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC4'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC5'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC6'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC7'].iloc[i])
            questions_GCPS.append(df_TRAITEMENT['DC8'].iloc[i])

            TREATMENT_predicted.append(predictTreatment(Y_predicted[i], df_TRAITEMENT['MD1'].iloc[i], df_TRAITEMENT['QS5'].iloc[i],  questions_OBC, questions_GAD, questions_PHQ9, questions_GCPS))
    ...
    package = {}
    package['Diagnostics'] = results
    package['Treatment'] = TREATMENT_predicted
    return package
