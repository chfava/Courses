...
class ELFMan8ViewController: QuestionnaireAbstractViewController {
    ...
    override func viewDidLoad() {
        ...
        if medicalExam != nil {
            self.questionnaire = medicalExam.examResult.listQuestionnaire.questionnaires["ELFMAN8"]
            self.setAnswers(exportedData: medicalExam.examResult.listQuestionnaire.questionnaires["ELFMAN8"]?.exportedData ?? [:])
    ...
    override func nextButtonTapped(sender: UIButton!) {
        if calculateScore() {
            self.nextQuestionnaireID = "ELFMAN20"
        }
        if (checkAllInput) {
            if(checkNext()){
